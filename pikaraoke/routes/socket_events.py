...
def setup_socket_events(socketio):
    ...
    def record_score(data: dict) -> None:
        """Persist the score generated by the elected splash screen."""
        if request.sid != master_splash_id or not isinstance(data, dict):
            return
        try:
        ...
        if not 0 <= score <= 99 or not controller.now_playing_url:
            return
        recorded = k.db.record_score(
            session_id=k.score_session_id,
            playback_id=controller.now_playing_url,
            participant=controller.now_playing_user or "Anonymous",
            song=controller.now_playing or "Unknown song",
            score=score,
        )
        if recorded:
            socketio.emit("score_update", namespace="/")
